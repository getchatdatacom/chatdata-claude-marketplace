...
2. Frame the work before analyzing: decision, metric, grain, period, segment, source of truth, owner, hypothesis, and acceptable confidence.
...
8. Run a skeptical validation pass before saying an answer path is ready, trusted, fixed, or reusable: rederive key numbers, check filters and joins, name the most likely failure, and downgrade the trust label if proof is incomplete.
9. When the analysis creates reusable knowledge, save it through MCP with the smallest write tool: `chatdata_create_metric_card`, `chatdata_save_answer_path`, `chatdata_create_proof_receipt`, or `chatdata_propose_patch`.
10. Recommend builder-mode follow-ups when a gap belongs in the trust layer rather than another ad hoc answer.
