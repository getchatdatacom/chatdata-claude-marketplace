...
- Use `chatdata_save_answer_path` to submit a recurring question, canonical route, SQL or retrieval path, raw SQL SoT usage, verified dashboard/report tie-out, business-context check, validation check, caveat, and reuse rule to human review.
- Use `chatdata_create_proof_receipt` to submit install proof, design-partner workflow proof, benchmark tie-out proof, or first trusted answer proof to human review.
