...
- If the tool is unavailable, say: "ChatData plugin is installed, but ChatData MCP is not connected. Go to ChatData Settings or `https://getchatdata.com/agents.md`, copy the Claude Code setup prompt, install remote MCP plus the ChatData plugin, reload or restart Claude Code, then rerun `/chatdata:help`. The terminal setup command only repairs local `~/.chatdata/config.json`; it is not the full Claude Code setup path."
...
If the user needs detailed metric packet, answer path, proof receipt, quality review, warehouse-query, pilot security, or setup troubleshooting guidance, use `chatdata_list_guidance` and `chatdata_read_guidance` through MCP instead of expanding that guidance in this help output.
